...
urlpatterns = [
    ...
    path("creators/me/password/change/", dj_rest.PasswordChangeView.as_view(), name="creator_change_password"),
    
    
    # session authentication urls
    path("auth/login/session/", apis.LoginSessionApiView.as_view({'post': 'post'}), name="login_via_session"),
    path("auth/logout/session/", apis.LogoutSessionApiView.as_view({'post': 'post'}), name="logout_via_session"),
