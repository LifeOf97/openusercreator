...
class TestClass:
    ...
    @pytest.mark.django_db
    def test_appusermodel_username_field_is_unique(self, test_user_1, test_user_1_2):
        AppUser.objects.create_user(**test_user_1)

        with pytest.raises(db_exception.IntegrityError) as exc_info:
            AppUser.objects.create_user(**test_user_1_2)
    ...
    def test_appusermodel_email_field_is_unique_cannot_save_email_if_already_exists(self, test_user_1_2, test_user_2):
        ...
        with pytest.raises(db_exception.IntegrityError) as exc_info:
            AppUser.objects.create_user(**test_user_2)
